...
export default function TicketsPage() {
  ...
  useEffect(() => { const timer = setTimeout(load, 250); return () => clearTimeout(timer) }, [JSON.stringify(query)])
  useEffect(() => { Promise.allSettled([api.get('/categories/'), ...(tech ? [api.get('/sectors/'), api.get('/technicians/', { params: { page_size: 100 } })] : [])]).then((responses) => setOptions({ categories: rowsOf(responses[0]?.value?.data), sectors: rowsOf(responses[1]?.value?.data), users: rowsOf(responses[2]?.value?.data) })) }, [tech])
